...
void SGMStereo::compute( const cv::Mat& leftImage,
						...
						float* disparityImage, const int height, const int width)
{
	height_ = height;
	width_=width;
...
void SGMStereo::convertToGrayscale(const cv::Mat& leftImage,
		const cv::Mat& rightImage,
								   ...
								   unsigned char* rightGrayscaleImage) const
{
	...
	for (int y = 0; y < height_*width_*3;y=y+3) {
			leftGrayscaleImage[idx] = static_cast<unsigned char>(0.299*leftImage.data[y] + 0.587*leftImage.data[y+1] + 0.114*leftImage.data[y+2] + 0.5);
			rightGrayscaleImage[idx++] = static_cast<unsigned char>(0.299*rightImage.data[y] + 0.587*rightImage.data[y+1] + 0.114*rightImage.data[y+2] + 0.5);
	}
}
