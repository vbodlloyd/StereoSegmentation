		...
		std::vector<std::vector<int> > &boundaryLabels)
{
	...
	//alloue de l'espace memoire pour toutes les variables
	allocateBuffer();


	//créer une image lab a partir de l'image de gauche et lance un algo de semi-global matching dont la réponse est dans initialDisparityImage_
	setInputData(leftImage, rightImage);

	//initialize les segments grace a l'image d'origine puis une interpolé. Flag les outliers et les boundary.
	initializeSegment(superpixelTotal);

	// assigne les boundaries au segment, change la disparity plane des segments
	performSmoothingSegmentation();

	//fait les images de sortie ( segmentImage, disparityImage )
...
void SPSStereo::initializeSegment(const int superpixelTotal) {
	//remplie segments_ avec des segments créer à partir de l'image lab.
	// et remplie labelImage avec l'index du segment associé au pixel
	//segments_ contient ( pour une image de 752 * 480 et 1000 superpixels ) 1000 segments
	// segments_ est un vecteur de segment.
	makeGridSegment(superpixelTotal);

	//remplie le boundaryFlagImage_ avec des 1 et des 0 ( 1 = boundary, 0 = not boundary ) grace au index des segments
	//et change les pixels de segment
	assignLabel();

	//remplie la disparity par segment et met un flag sur chaque pixel pour savoir si c'est un outlier ou inlier
...
void SPSStereo::assignLabel() {
	...
	//on extrait les pixels de coin en regardant si ses points voisins ne se trouvent pas dans le meme segment.
	...
	while (!boundaryPixelIndices.empty()) {
		...
		//si le pixel a un voisin en dehors de l'image, on continue.
		if (isUnchangeable(pixelX, pixelY)) continue;

		//on choppe le segment qui a le moins d'energie, si c'est celui qu'on pointe déja, on continue
		int bestSegmentIndex = findBestSegmentLabel(pixelX, pixelY);
		if (bestSegmentIndex == labelImage_[width_*pixelY + pixelX]) continue;

		//on change à quel segment appartient le pixel
		changeSegmentLabel(pixelX, pixelY, bestSegmentIndex);
		//et on rajoute le flag boundary au voisin
...
int SPSStereo::findBestSegmentLabel(const int x, const int y) const {
	...
	//choppe les index des voisins s'il y en a, avec en [0] l'indice du segment en x,y. Donc on passe forcement dans la boucle d'aprés
...
void SPSStereo::initialFitDisparityPlane() {
	//utilise RANSAC pour faire la moyenne des pixels dans un segment et définir la disparityPlane de chaque segments de segments_
	...
	//on interpole la disparity image d'origine, cad quand la disparité d'origine = 0  on regarde les pixels a coté et on interpole un valeur pour la disparité
	...
	//initialise le tableau d'outlier, un flag par pixel, si la disparité de l'image d'origine - celles faite juste en haut dépasse inlierTreshold, on met le flag a 255
...
void SPSStereo::planeSmoothing() {
	for (int innerIterationCount = 0; innerIterationCount < innerIterationTotal_; ++innerIterationCount) {
		//set le type de chaque boundary ( 1 2 ou 3 )
		estimateBoundaryLabel();
		//set la disparity plane de chaque segments
